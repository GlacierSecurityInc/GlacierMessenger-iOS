...
@interface OTRComposeViewController () <UITableViewDataSource, UITableViewDelegate, OTRYapViewHandlerDelegateProtocol, UISearchResultsUpdating, UISearchControllerDelegate, UISearchBarDelegate, OTRComposeGroupViewControllerDelegate, OTRJoinGroupViewControllerDelegate>
...
- (void)viewDidLoad
{
    ...
    [cancelBarButtonItem setTintColor:lblColor]; //was white
    [self.groupBarButtonItem setTintColor:lblColor];
...
- (void) queryMissingKeys {
    ...
    __block NSMutableArray<XMPPJID*> *budjids = [[NSMutableArray alloc] init];
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    hud.label.text = @"Fetching secure keys";
    
    [OTRDatabaseManager.shared.uiConnection readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        NSInteger num = [self.viewHandler.mappings numberOfItemsInSection:0];
        for (int i = 0; i < num; i++) {
            NSIndexPath *viewIndexPath = [NSIndexPath indexPathForItem:i inSection:0];
            id<OTRThreadOwner> budthread = [self.viewHandler object:viewIndexPath];
            OTRXMPPBuddy *xbud = (OTRXMPPBuddy *)budthread;
        
            NSArray<OMEMODevice*> *devices = [OMEMODevice allDevicesForParentKey:xbud.uniqueId collection:OTRXMPPBuddy.collection transaction:transaction];
            if (devices == nil || devices.count == 0) {
                [budjids addObject:xbud.bareJID];
            }
        }
    }];
    ...
    OTRXMPPAccount *xacct = (OTRXMPPAccount *)accounts.firstObject;
    OTRXMPPManager *xmpp = (OTRXMPPManager*)[OTRProtocolManager.shared protocolForAccount:xacct];
    if (xmpp != nil) {
        [budjids enumerateObjectsUsingBlock:^(XMPPJID * _Nonnull jid, NSUInteger idx, BOOL * _Nonnull stop) {
            [xmpp.omemoSignalCoordinator.omemoModule fetchDeviceIdsForJID:jid elementId:nil];
        }];
    }
    //remove indeterminate progress after a few seconds
...
- (void) groupButtonPressed:(id)sender {
    ...
    NSMutableAttributedString *atrStr = [[NSMutableAttributedString alloc] initWithString:@"\nGroups are invite only, have a 10 user recommended limit, and are managed by the owner" attributes:@{NSParagraphStyleAttributeName:paraStyle,NSFontAttributeName:[UIFont systemFontOfSize:13.0]}];
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"New Group" message:@"Groups are invite only, have a 10 user recommended limit, and are managed by the owner" preferredStyle:UIAlertControllerStyleAlert];
    [alert setValue:atrStr forKey:@"attributedMessage"];
    UIAlertAction *privateAction = [UIAlertAction actionWithTitle:@"Create Group" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"OTRComposeGroup" bundle:[GlacierInfo resourcesBundle]];
        OTRComposeGroupViewController *vc = (OTRComposeGroupViewController *)[storyboard instantiateInitialViewController];
        UITextField *firstTextField = alert.textFields.firstObject;
        NSString *name = firstTextField.placeholder;
        if (firstTextField.text.length > 1) {
            name = firstTextField.text;
        }
        
        if ([self groupAlreadyExists:name]) {
            [self handleGroupExists:alert];
        } else {
            vc.groupName = name;
            vc.delegate = self;
    ...
    UIAlertAction *publicAction = [UIAlertAction actionWithTitle:@"Open" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        UITextField *firstTextField = alert.textFields.firstObject;
        NSString *name = firstTextField.placeholder;
        if (firstTextField.text.length > 1) {
            name = firstTextField.text;
        ...
        // check for existing groups of the same name
        if ([self groupAlreadyExists:name]) {
            [self handleGroupExists:alert];
        } else {
            [self.delegate controllerDidCancel:self];
            [self.delegate enterPublicConversationWithName:name];
        }
    }];
    
    [alert addAction:privateAction];
    [alert addAction:[UIAlertAction actionWithTitle:CANCEL_STRING() style:UIAlertActionStyleCancel handler:nil]];
...
- (void) handleGroupExists:(UIAlertController *)alert {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Group Exists" message:@"Group already exists, please join the existing group or select a different name." preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancel = [UIAlertAction actionWithTitle:OK_STRING() style:UIAlertActionStyleCancel handler:nil];
...
- (BOOL) groupAlreadyExists:(NSString *)groupName {
    NSArray *accounts = [OTRAccountsManager allAccounts];
    if ([accounts count] == 0) {return NO;}
    OTRXMPPAccount *xacct = (OTRXMPPAccount *)accounts.firstObject;
    OTRXMPPManager *xmpp = (OTRXMPPManager*)[OTRProtocolManager.shared protocolForAccount:xacct];
    if (xmpp != nil) {
        NSArray<NSString*> *groups = xmpp.roomManager.allRooms;
        if (groups == nil) { return NO; }
        __block BOOL exists = NO;
        [groups enumerateObjectsUsingBlock:^(NSString *name, NSUInteger idx, BOOL *stop) {
            NSString *slimName = [name componentsSeparatedByString:@"@"].firstObject;
            if ([groupName isEqualToString:slimName]) {
                exists = YES;
                *stop = YES;
            }
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    __block OTRXMPPBuddy *xbud = (OTRXMPPBuddy *)threadOwner;
    ...
    [OTRDatabaseManager.shared.uiConnection readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        ...
        // gray and not selectable if no bundle/keys
        devices = [OMEMODevice allDevicesForParentKey:xbud.uniqueId collection:OTRXMPPBuddy.collection transaction:transaction];
    }];
    
    [cell setThread:threadOwner account:account];
    
    if (devices == nil || devices.count == 0) {
        [cell setUserInteractionEnabled:NO]; //not selectable, gray
        cell.accountLabel.text = @"No secure keys available";
        cell.accountLabel.textColor = [UIColor grayColor];
...
- (void) joinGroup:(id)sender {
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"OTRJoinGroup" bundle:[GlacierInfo resourcesBundle]];
    OTRJoinGroupViewController *vc = (OTRJoinGroupViewController *)[storyboard instantiateInitialViewController];
    vc.delegate = self;
    [self.navigationController pushViewController:vc animated:YES];
}
...
#pragma - mark OTRJoinGroupViewControllerDelegate
- (void)groupSelected:(OTRJoinGroupViewController *)composeViewController groupName:(NSString *)groupName {
    if (!groupName) {
        return;
    }
    ...
    NSArray *components = [groupName componentsSeparatedByString:@"@"];
    if (components.count != 2) {
        NSString *after = [NSString stringWithFormat: @"conference.%@", xacct.domain];
        groupName = [NSString stringWithFormat: @"%@@%@", groupName, after];
    }
    ...
    void (^joinRoom)(OTRAccount *account) = ^void(OTRAccount *account) {
        ...
        if (!xmpp) { return; }
        [xmpp.roomManager joinRoom:roomJid withNickname:account.displayName subject:nil password:nil];
        [xmpp.roomManager publishJoinedRoom:roomJid withName:account.displayName];
...
- (void)joinGroupCancelled:(OTRJoinGroupViewController *)composeViewController {
}
