...
@interface OTRSettingsViewController () <UITableViewDataSource, UITableViewDelegate, OTRYapViewHandlerDelegateProtocol,OTRSettingDelegate, UIPopoverPresentationControllerDelegate, OTRAttachmentPickerDelegate, UIPickerViewDataSource,UIPickerViewDelegate, UIGestureRecognizerDelegate>
...
-(void) doLogout:(id)sender {
    ...
    if (account && [[OTRProtocolManager sharedInstance] existsProtocolForAccount:account]) {
        ...
        if (account.pushPubsubEndpoint) {
            ...
            //log out of push. This will use the device and should impact other devices with same user
            XMPPJID *userjid = [XMPPJID jidWithString:account.username resource:account.resource];
            NSString *pushjid = [@"p2." stringByAppendingString:[GlacierInfo defaultHost]];
            [xmpp.serverCheck.pushModule unregisterPushForUserJID:userjid server:pushjid elementId:nil];
        }
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == [self.viewHandler.mappings numberOfItemsInSection:indexPath.section]) {
            ...
            if (cell == nil) {
                ...
                cell.imageView.image = [UIImage imageNamed:circleImageName inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil];
...
- (void) otrSetting:(OTRSetting*)setting showDetailViewControllerClass:(Class)viewControllerClass
{
    if (viewControllerClass == [EnablePushViewController class]) {
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Onboarding" bundle:[GlacierInfo resourcesBundle]];
        ...
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"CoreConnection" bundle:[GlacierInfo resourcesBundle]];
        ...
        return;
    } else if (viewControllerClass == [ProfileViewController class]) {
        ProfileViewController *viewController = (ProfileViewController *)[[viewControllerClass alloc] init];
