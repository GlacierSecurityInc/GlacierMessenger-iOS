...
- (BOOL) needsLocationPermission {
    BOOL needsPermission = NO;
    if ([CLLocationManager locationServicesEnabled]) {
        switch ([CLLocationManager authorizationStatus]) {
            case kCLAuthorizationStatusNotDetermined:
                needsPermission = YES;
                [self handleLocationAuthorizationNotDetermined];
                break;
            case kCLAuthorizationStatusRestricted:
            case kCLAuthorizationStatusDenied:
                needsPermission = YES;
                [self handleLocationAuthorizationDenied];
                break;
            default:
                needsPermission = NO;
                break;
...
- (void) handleLocationAuthorizationNotDetermined {
    CLLocationManager *locMgr = [[CLLocationManager alloc] init];
    [locMgr requestWhenInUseAuthorization];
}

- (void) handleLocationAuthorizationDenied {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Allow Location Access" message:@"Whoops! It looks like you have not permitted Glacier to access your Location. To use this feature, please turn on Location Services while using the app in your device Glacier settings." preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *fix = [UIAlertAction actionWithTitle:@"Enable" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSURL *settings = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
        [[UIApplication sharedApplication] openURL:settings options:@{} completionHandler:nil];
    }];
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:CANCEL_STRING() style:UIAlertActionStyleCancel handler:nil];
...
{
    if ([self needsLocationPermission]) {
        return;
    }
    ...
    //self.mapView.userLocation.subtitle = @"The coolest ever";
...
- (void)addLocationViewBtns:(BOOL)shareable
{
    ...
    self.locbar.barTintColor = GlobalTheme.shared.lightThemeColor;
    self.locbar.tintColor = lblColor;
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView didTapMessageBubbleAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([item isKindOfClass:[OTRImageItem class]]) {
        ...
        if (origtext && [origtext hasPrefix:@"geo"]) {
            [self findBuddyLocation:origtext withMessage:message];
        } else {
...
- (void) findBuddyLocation:(NSString *)buddyloc withMessage:(id <OTRMessageProtocol,JSQMessageData>)message{
    if ([self needsLocationPermission]) {
        return;
    }
    ...
    if ([self isGroupChat] && [message isKindOfClass:[OTRXMPPRoomMessage class]]) {
        OTRXMPPRoomMessage *roommsg = (OTRXMPPRoomMessage *)message;
        if (roommsg.senderJID != nil) {
            XMPPJID *jid = [XMPPJID jidWithString:roommsg.senderJID];
            friendloc.title = jid.resource;
        } else {
            friendloc.title = [self getThreadName];
        }
    } else if ([self getThreadName]) {
        friendloc.title = [self getThreadName];
    }
...
- (void) updateTimeButton {
    ...
    {
        case 1:
            [self.timeButton setImage:[UIImage imageNamed:@"timer_15s_blue" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            break;
        case 2:
            [self.timeButton setImage:[UIImage imageNamed:@"timer_1m_blue" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            break;
        case 3:
            [self.timeButton setImage:[UIImage imageNamed:@"timer_5m_blue" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            break;
        case 4:
            [self.timeButton setImage:[UIImage imageNamed:@"timer_1d_blue" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            break;
        case 5:
            [self.timeButton setImage:[UIImage imageNamed:@"timer_1w_blue" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            break;
        default:
            break;
    }
