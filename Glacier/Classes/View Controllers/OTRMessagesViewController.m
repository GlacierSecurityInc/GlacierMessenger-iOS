...
@property (strong, nonatomic) NSMutableSet<NSString *> *cachedImageIds;
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
        ...
        self.cachedImageIds = [[NSMutableSet alloc] init];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"OTRMessagesLoadingView" bundle:GlacierInfo.resourcesBundle]
...
- (void)didReceiveMemoryWarning
{
    ...
    [self.cachedImageIds enumerateObjectsUsingBlock:^(NSString * _Nonnull imageId, BOOL * _Nonnull stop) {
        [OTRImages removeImageWithIdentifier:imageId];
    }];
    [self.cachedImageIds removeAllObjects];
}
...
- (void)setThreadKey:(NSString *)key collection:(NSString *)collection
{
    ...
    if (![self isGroupChat]) {
        ...
        [self fetchOMEMODeviceListForRoom];
    }
}
...
- (void)tryToMarkAllMessagesAsRead {
    ...
    if ([self otr_isVisible]) {
        ...
        [self.connections.read asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
            ...
            __block id<OTRMessageProtocol> needsReadReceipt = nil;
            ...
            // this only sends to the last person
            if (needsReadReceipt != nil) {
                [xmpp sendReadReceipt:needsReadReceipt];
            }
...
- (void)setupInfoButton {
    ...
    phoneButton.contentEdgeInsets = UIEdgeInsetsMake(4, 8, 4, 8);
    phoneButton.imageEdgeInsets = UIEdgeInsetsMake(4, 8, 4, 8);
    phoneButton.translatesAutoresizingMaskIntoConstraints = NO;
    phoneButton.frame = CGRectMake(0, 0, 64, 48);
    phoneButton.titleLabel.font = [UIFont fontWithName:kFontAwesomeFont size:26];
    phoneButton.titleLabel.textAlignment = NSTextAlignmentCenter;
    [phoneButton setTintColor:lblColor];
    [phoneButton setTitleColor:lblColor forState:UIControlStateNormal];
    [phoneButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    [phoneButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
    [phoneButton setTitle:[NSString fa_stringForFontAwesomeIcon:FAIconPhone] forState:UIControlStateNormal];
    [phoneButton addTarget:self action:@selector(didSelectPhoneButton:) forControlEvents:UIControlEventTouchUpInside];
    
    if ([self isGroupChat]) {
        UIBarButtonItem *phoneItem = [[UIBarButtonItem alloc] initWithCustomView:phoneButton];
        if ([OTRAppDelegate appDelegate].callWindow != nil && [OTRAppDelegate appDelegate].callWindow.rootViewController != nil) {
        ...
        UIImage *origImage = [UIImage imageNamed:@"112-group" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil];
        UIImage *normalImage = [origImage jsq_imageMaskedWithColor:lblColor];
        ...
        NSArray *items = [NSArray arrayWithObjects:groupItem, phoneItem, nil];
...
- (void)didSelectPhoneButton:(id)sender {
    
    __block OTRXMPPManager *xmpp = nil;
    __block id <OTRThreadOwner> owner = nil;
    __block NSArray<NSString*> *callBuddies;
    ...
    [self.connections.ui readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        ...
        if ([self isGroupChat]) {
            ...
            if (room) {
                NSArray<OTRXMPPRoomOccupant*> *allOccs = [room allOccupants:transaction];
                NSMutableArray<NSString*> *callBuddiesArray = [[NSMutableArray alloc] initWithCapacity:[allOccs count]];
                [allOccs enumerateObjectsUsingBlock:^(OTRXMPPRoomOccupant * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    OTRXMPPBuddy *roomOccupantBuddy = [obj buddyWith:transaction];
                    if (roomOccupantBuddy != nil && ![roomOccupantBuddy.username isEqualToString:xmpp.account.username]) {
                        [callBuddiesArray addObject: roomOccupantBuddy.username];
                    }
                }];
                callBuddies = [callBuddiesArray copy];
                owner = room;
            }
        } else {
            OTRBuddy *buddy = [self buddyWithTransaction:transaction];
            if (buddy != nil) {
                callBuddies = [NSArray arrayWithObjects:buddy.username,nil];
                owner = buddy;
            }
        }
    }];
    ...
    if (callBuddies.count > 4) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Unable to Call" message:@"Calls are limited to five participants." preferredStyle:UIAlertControllerStyleAlert];
    ...
    if ([xmpp.callManager checkAudioPermissions]) {
        [[AVAudioSession sharedInstance] requestRecordPermission:^(BOOL granted) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (granted) {
                    return;
                }
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Allow Audio" message:@"Whoops! It looks like you have not permitted Glacier to access your Microphone. To use Glacier calling, please turn on Microphone while using the app in your device Glacier settings." preferredStyle:UIAlertControllerStyleAlert];
            
                UIAlertAction *enableAction = [UIAlertAction actionWithTitle:@"Enable" style:UIAlertActionStyleDefault
    ...
    // prevent accidental hot mic
    UIAlertAction *makeCall = [UIAlertAction actionWithTitle:@"Call" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [xmpp.callManager makeCall:callBuddies name:[self getThreadName]];
        [self addCallUpdate:@"You called" withMgr:xmpp withOwner:owner];
    }];
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:CANCEL_STRING() style:UIAlertActionStyleCancel handler:nil];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Start Call?" message:nil preferredStyle:UIAlertControllerStyleAlert];
    
    [alert addAction:makeCall];
    [alert addAction:cancel];
    
    [self presentViewController:alert animated:YES completion:nil];
}

- (void) addCallUpdate:(NSString *)updateMsg withMgr:(OTRXMPPManager *)xmpp withOwner:(id <OTRThreadOwner>)owner {
    if (!xmpp || !owner) {
        return;
    }
    
    [self.connections.write asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
        id<OTRMessageProtocol> message = [owner outgoingMessageWithText:updateMsg transaction:transaction];
        message.messageSecurity = OTRMessageTransportSecurityPlaintext;
        owner.lastMessageIdentifier = message.messageKey;
        ...
        if ([message isKindOfClass:[OTROutgoingMessage class]]) {
            ...
            systemMsg.originalText = updateMsg;
        ...
        [owner saveWithTransaction:transaction];
    }];
}

- (void)enablePhoneButton:(BOOL)enable {
    if (self.navigationItem != nil && self.navigationItem.rightBarButtonItem != nil) {
...
- (void)didSelectOccupantsButton:(id)sender {
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"OTRRoomOccupants" bundle:[GlacierInfo resourcesBundle]];
...
-(void) fetchOMEMODeviceListForRoom {
    __block OTRAccount *account = nil;
    __block OTRXMPPManager *xmpp = nil;
    __block NSMutableArray<XMPPJID*> *roomBuddies = [[NSMutableArray alloc] init];
    [self.connections.ui readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        account = [self accountWithTransaction:transaction];
        xmpp = [self xmppManagerWithTransaction:transaction];
        OTRXMPPRoom *room = [self roomWithTransaction:transaction];
        if (room) {
            ...
            [allOccs enumerateObjectsUsingBlock:^(OTRXMPPRoomOccupant * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                OTRXMPPBuddy *roomOccupantBuddy = [obj buddyWith:transaction];
                if (roomOccupantBuddy != nil && ![roomOccupantBuddy.username isEqualToString:account.username]) {
                    [roomBuddies addObject:roomOccupantBuddy.bareJID];
                }
            }];
        }
    }];
    
    [roomBuddies enumerateObjectsUsingBlock:^(XMPPJID * _Nonnull jid, NSUInteger idx, BOOL * _Nonnull stop) {
        [xmpp.omemoSignalCoordinator.omemoModule fetchDeviceIdsForJID:jid elementId:nil];
    }];
}
...
- (id <OTRMessageProtocol,JSQMessageData>)messageAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // I have an NSFetchedResultsController that's returning an index path with a row index of -1. So it might be worth checking for >= 0 as well as nil (and maybe NSNotFound on item)
    if (indexPath != nil && indexPath.row >= 0 && ![indexPath isEqual:self.currentIndexPath]) {
...
- (BOOL)showDateAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([currentMessage isKindOfClass:[OTRXMPPRoomMessage class]]) {
        return showDate;
    } else if ([self isUpdateMessage:currentMessage]) {
        return YES;
    }
...
- (BOOL)showSenderDisplayNameAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self isMemberOrSystemUpdateMessageAtIndexPath:indexPath]) {
        return NO;
    }
...
- (BOOL)isMemberOrSystemUpdateMessageAtIndexPath:(NSIndexPath *)indexPath {
    id message = [self messageAtIndexPath:indexPath];
    return [self isUpdateMessage:message];
}

- (BOOL)isUpdateMessage:(id <OTRMessageProtocol>)message {
    if ([message isKindOfClass:[OTRXMPPRoomMessage class]]) {
        OTRXMPPRoomMessage *xmessage = (OTRXMPPRoomMessage *)message;
        return xmessage.memberUpdate || xmessage.systemUpdate;
    } else if ([message isKindOfClass:[OTRBaseMessage class]]){
        OTRBaseMessage *xmessage = (OTRBaseMessage *)message;
        return xmessage.systemUpdate;
    }
    return NO;
}
...
- (BOOL)isFailedDecryptMessage:(id <OTRMessageProtocol>)message {
    if ([[message messageText] hasPrefix:@"Couldn't decrypt"] || [[message messageText] hasPrefix:@"Glacierbot"] || [[message messageText] hasPrefix:@"Message cannot be"]) {
        return YES;
    }
    return NO;
}
...
- (void)keyboardController:(JSQMessagesKeyboardController *)keyboardController keyboardDidChangeFrame:(CGRect)keyboardFrame
{
    ...
    OTRAppDelegate *appDelegate = [OTRAppDelegate appDelegate];
    CGSize screenSize = [[UIScreen mainScreen] bounds].size;
    
    if (appDelegate.callWindow != nil && !CGRectEqualToRect(appDelegate.callWindow.frame, CGRectZero) && appDelegate.callWindow.frame.size.height < screenSize.height) { //call in progress and typing
    
        CGFloat heightFromBottom = CGRectGetMaxY(self.collectionView.frame) - CGRectGetMinY(keyboardFrame);
        heightFromBottom = MAX(0.0, heightFromBottom);
        CGFloat safetop = self.view.safeAreaInsets.top;
        if (IS_IPHONE) { safetop += IPHONE_STATUSBAR_HEIGHT; }
        if (heightFromBottom > 0.0) {
            [self jsq_setToolbarBottomLayoutGuideConstant:heightFromBottom+safetop+44];
        }
    }
}
        ...
        [self.connections.ui readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
            ...
            if (!thread.isGroupThread) {
                if ([messageItem.text hasPrefix:@"https"] || [messageItem.text hasPrefix:@"aesgcm"]) {
                    ...
                    if (urlComponents != nil && urlComponents.count > 1) {
                        NSString* name = urlComponents[1];
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isFailedDecryptAtIndexPath:indexPath]) {
        [cell.textView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleFailedDecryptGesture)]];
    ...
    NSString *mediaKey = [message messageMediaItemKey];
    if (mediaKey != nil) {
        [self.cachedImageIds addObject:mediaKey];
    }
...
{
    id <OTRMessageProtocol,JSQMessageData> message = [self messageAtIndexPath:indexPath];
    if ([self isUpdateMessage:message]) {
        return nil;
    }
...
- (NSAttributedString *)collectionView:(JSQMessagesCollectionView *)collectionView attributedTextForCellTopLabelAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isMemberOrSystemUpdateMessageAtIndexPath:indexPath]) {
        ...
        if (updateString != nil) {
            updateString = [self getFormattedSystemOrMemberUpdate:updateString];
...
- (NSString *) getFormattedSystemOrMemberUpdate:(NSString *)updateString {
...
- (nullable NSAttributedString*) deliveryStatusStringForMessage:(nonnull id<OTRMessageProtocol>)message {
    ...
    } else if (message.isMessageDisplayed) { 
        deliveryStatusString = [NSString stringWithFormat:@"%@ %@ ",[NSString fa_stringForFontAwesomeIcon:FAIconCheck], @"Read"];
...
- (void) handleConnectingStatus:(BOOL)protocolExists {
    ...
    if (_statusCtr < 2) {
        ...
        if (networkStatus == OTRNetworkConnectionStatusConnected) { // logging in
            ...
            self.titleView.subtitleLabel.text = @"Waiting for Core Connection";
...
- (void) handleOfflineNetworkMessage {
    ...
    if ([VPNManager.shared vpnIsEnabled]) {
        self.titleView.subtitleLabel.text = @"Offline - Enable Core Connection in Settings";
    } else {
...
- (void) handleFailedDecryptGesture {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Reset Sessions" message:@"The received message was out of sync and cannot be decrypted. Resetting sessions and sending a message will fix this for future messages. This error does not affect message security." preferredStyle:UIAlertControllerStyleAlert];
...
- (void) doResetSecureConnections {
    ...
    if (account != nil && xmpp != nil) {
        ...
        OMEMODevice *currentDevice = [[OMEMODevice alloc] initWithDeviceId:idNumber trustLevel:OMEMOTrustLevelTrustedUser parentKey:account.uniqueId parentCollection:OTRAccount.collection publicIdentityKeyData:bundle.identityKey lastSeenDate:[xmpp getLastConnected]];
        
        if (currentDevice != nil) {
            NSArray<OMEMODevice *>* mydevices = @[currentDevice];
...
{
    if (_statusCtr < 22 && self.showConnStatus) { 
        _statusCtr++;
    } else {
...
- (void)showJIDForwardingHeaderWithNewJID:(XMPPJID *)newJid {
    if (self.jidForwardingHeaderView == nil) {
        UINib *nib = [UINib nibWithNibName:@"MigratedBuddyHeaderView" bundle:GlacierInfo.resourcesBundle];
