...
+ (UIView *)typingBubbleView
{
    ...
    bubbleImage = [UIImage imageNamed:@"bubble-min-tailless" inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil];
...
{
    ImageCache *ic = [[self imageCache] objectForKey:identifier];
    if (ic) {
        return ic.image;
    } else {
        return nil;
...
+ (void)setImage:(UIImage *)image forIdentifier:(NSString *)identifier
{
    ...
    if (image && [image isKindOfClass:[UIImage class]]) {
        ImageCache *cacheImage = [[ImageCache alloc] init];
        cacheImage.image = image;
        
        [[self imageCache] setObject:cacheImage forKey:identifier];
