    ...
    @objc public func getUserInfo(username: String, org: String, completion:@escaping (GlacierUser?) -> Void)  {
        ...
        client.fetch(query: gquery)  { (result, error) in
            ...
            if let pw = result?.data?.getGlacierUsers?.glacierpwd,
                ...
                if let firstname = result?.data?.getGlacierUsers?.firstName {
                    displayName = firstname
                    if let lastname = result?.data?.getGlacierUsers?.lastName {
                ...
                let guser = GlacierUser(username: user, voiceext: ext ?? "", password: pw, org: lowerorg, displayname: displayName)
                completion(guser)
            } else {
                completion(nil)
            }
...
@objc public class GlacierUser:NSObject {
    ...
    @objc public var voiceext:String?
