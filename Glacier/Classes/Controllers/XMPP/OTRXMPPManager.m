...
@property (assign, nonatomic) BOOL enablingPush;

@property (strong, nonatomic) NSMutableSet *teams;
@property (strong, nonatomic) NSDate *lastConnected;
@end
...
- (void)setupStream
{
    ...
    [_callManager setAccount:self.account];
    [self.callManager activate:self.xmppStream];
    ...
    BOOL allowselfsigned = NO;
    NSString *uploaddomain = [GlacierInfo defaultHost];
    if (_account != nil) {
        uploaddomain = _account.domain;
    }
    
    _fileTransferManager = [[FileTransferManager alloc] initWithConnection:self.databaseConnection serverCapabilities:self.serverCheck.serverCapabilities sessionConfiguration:sessionConfiguration allowSelfSigned:allowselfsigned uploadDomain:uploaddomain];
...
- (void)teardownStream
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:OTRPushAccountDeviceChanged object:OTRProtocolManager.pushController];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:OTRPushAccountTokensChanged object:OTRProtocolManager.pushController];
    ...
    [_serverCheck.xmppCapabilities removeDelegate:self];
    [_serverCheck.xmppCapabilities removeDelegate:self.omemoModule];
    [_serverCheck.xmppCapabilities removeDelegate:self.roomManager];
    [_xmppRoster removeDelegate:self.xmppRosterStorage];
    [_streamManagement removeDelegate:self.streamManagementDelegate];
    [_omemoModule removeDelegate:self.omemoSignalCoordinator];
    [_fileTransferManager removeDelegates];
    ...
    [_callManager deactivate];
    _callManager = nil;
    
    [_stanzaIdModule deactivate];
    //[_roomManager deactivate];  // done in message storage deactivate
    _xmppPushModule = nil;
...
- (void) addIdleDate:(NSDate*)date toPresence:(XMPPPresence*)presence {
    // Don't leak any extra info over non-autologin accounts
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([UIApplication sharedApplication].applicationState == UIApplicationStateActive) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                if ([UIApplication sharedApplication].applicationState == UIApplicationStateActive) {
                    [self goActive];
                } else if ([_callManager isRinging]) {
                    [self goActive];
                } else {
                    DDLogError(@"Application state not active so presence online will not be sent and 1:1 messages will not be received.");
                }
            });
        }
    });
...
- (void)goOffline
{
    [OTRAppDelegate getLastInteractionDate:^(NSDate * _Nullable date) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), self.workQueue, ^{
                [self.xmppStream disconnectAfterSending];
        });
    } completionQueue:self.workQueue];
}

- (NSDate *) getLastConnected {
...
- (void)authenticateWithStream:(XMPPStream *)stream {
    ...
    if ([stream supportsXOAuth2GoogleAuthentication] && self.account.accountType == OTRAccountTypeGoogleTalk) {
        ...
        if (self.account.password == nil) {
            //[GlacierLog glogWithLogMsg:@"authenticateWithStream nil password" detail:self.account.uniqueId];
        }
...
- (void) disconnectSocketOnly:(BOOL)socketOnly {
    ...
    [self.databaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction *transaction) {
        ...
        [buddiesArray enumerateObjectsUsingBlock:^(OTRBuddy * _Nonnull buddy, NSUInteger idx, BOOL * _Nonnull stop) {
            [strongSelf disconnectBuddy:buddy];
        }];
    }];
}

- (void)disconnectBuddy:(OTRBuddy *)buddy
{
    ...
    if (buddy) {
        ...
        [OTRBuddyCache.shared setStatusMessage:OFFLINE_STRING() forBuddy:newBuddy];
        [OTRBuddyCache.shared setChatState:OTRChatStateGone forBuddy:newBuddy];
    }
    
    [OTRBuddyCache.shared setThreadStatus:newStatus forBuddy:newBuddy resource:nil];
}
...
- (void)xmppStreamDidDisconnect:(XMPPStream *)sender withError:(NSError *)error
{
    ...
    [self.databaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction *transaction) {
        ...
        [allBuddies enumerateObjectsUsingBlock:^(OTRBuddy * _Nonnull buddy, NSUInteger idx, BOOL * _Nonnull stop) {
            [self disconnectBuddy:buddy];
        }];
...
- (void)xmppStreamDidAuthenticate:(XMPPStream *)sender
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[OTRAppDelegate appDelegate] setResortHandler];
    });
...
- (void) updateNickname:(XMPPvCardTemp *)vCardTemp {
    if (vCardTemp.nickname.length) {
        ...
        self.account.displayName = vCardTemp.nickname;
        [self.xmppvCardTempModule updateMyvCardTemp:vCardTemp];
        
        XMPPPresence *presence = [XMPPPresence presence];
        NSXMLElement *xElement = [NSXMLElement elementWithName:@"x" xmlns:@"vcard-temp:x:update"];
        NSXMLElement *displayElement = [NSXMLElement elementWithName:@"displayname" stringValue:self.account.displayName];
        NSXMLElement *displayOnly = [NSXMLElement elementWithName:@"displayonly" stringValue:@"YES"];
        [xElement addChild:displayElement];
        [xElement addChild:displayOnly];
        [presence addChild:xElement];
        [self.xmppStream sendElement:presence];
    }
}

- (void) updateNetworkCheckBypass:(BOOL)bypass {
    [self.databaseConnection asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
        NSString *collection = [self.account.class collection];
        NSString *key = self.account.uniqueId;
        OTRXMPPAccount *account = [[transaction objectForKey:key inCollection:collection] copy];
        account.bypassNetworkCheck = bypass;
        [transaction setObject:account forKey:key inCollection:collection];
    }];
    self.account.bypassNetworkCheck = bypass;
}
...
- (void)xmppRoster:(XMPPRoster *)sender didReceiveRosterItem:(NSXMLElement *)item {
    ...
    NSXMLElement *group = [item elementForName:@"group"];
    if (group != nil) {
        [self.teams addObject:group.stringValue];
    }
    ...
    [self.databaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        ...
        [self.databaseConnection asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
            ...
            NSString *nameStr = [item attributeStringValueForName:@"name"];
            if (![nameStr isEqualToString:vCard.nickname]) {
                vCard.nickname = nameStr;
                buddy.displayName = nameStr;
            }
...
- (void)registerPushTokens {
    NSString *token = [OTRProtocolManager.pushController getPushToken];
    NSString *voiptoken = [OTRProtocolManager.pushController getVoipToken];
    if (token != nil || voiptoken != nil) { 
        XMPPJID *userjid = [XMPPJID jidWithString:self.account.username resource:self.account.resource];
        NSString *pushjid = [@"p2." stringByAppendingString:[GlacierInfo defaultHost]];
        
        [self.xmppPushModule handleDeviceTokenWithJID:userjid tojid:pushjid token:token voiptoken:voiptoken elementId:nil];
    }
}

/*- (void) prepareToEnablePush {
    [OTRProtocolManager.pushController getPubsubEndpoint:^(NSString * _Nullable endpoint, NSError * _Nullable error) {
        if (endpoint && !self.enablingPush) {
            self.enablingPush = YES;
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                self.enablingPush = NO;
            });
...
didRegisterWithResponseIq:(XMPPIQ*)responseIq
    ...
    // get node and server ID
    NSXMLElement *enable = [outgoingIq elementForName:@"enable" xmlns:XMPPPushXMLNS];
    if (enable == nil) {
        return;
    }
    
    __block NSString *serverjid = [enable attributeStringValueForName:@"jid"];
    __block NSString *node = [enable attributeStringValueForName:@"node"];
    ...
    [self.databaseConnection readWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
        ...
        account = [[transaction objectForKey:key inCollection:collection] copy];
        if (account != nil) {
            account.pushPubsubEndpoint = serverjid;
            account.pushPubsubNode = node;
            [transaction setObject:account forKey:key inCollection:collection];
        }
    }];
}
...
- (void) sendMessage:(OTROutgoingMessage*)message
{
    ...
    if (message.expires) {
        ...
        [expiresElement addAttributeWithName:@"seconds" stringValue:message.expires];
        [xmppMessage addChild:expiresElement];
    }
    
    if (message.systemUpdate) {
        NSXMLElement *updateElement = [NSXMLElement elementWithName:@"x" xmlns:@"jabber:x:systemupdate"];
        [xmppMessage addChild:updateElement];
    } else { 
...
- (OTRXMPPBuddy *)addToRosterWithJID:(XMPPJID *)jid displayName:(NSString *)displayName transaction:(YapDatabaseReadWriteTransaction *)transaction{
    ...
    if (buddy.displayName == nil || buddy.displayName.length == 0) {
        NSString *jidNode = nil; // aka 'username' from username@example.com
        NSArray *components = [buddy.username componentsSeparatedByString:@"@"];
        if (components.count == 2) {
            jidNode = [components firstObject];
        } else {
            jidNode = buddy.username;
        }
        
        if (!jidNode.length) {
            // strip whitespace and make nickname lowercase
            jidNode = [jidNode stringByReplacingOccurrencesOfString:@" " withString:@""];
            jidNode = [jidNode lowercaseString];
        }
        buddy.displayName = jidNode;
    }
...
- (void)newTrust:(SecTrustRef)trust withHostName:(NSString *)hostname systemTrustResult:(SecTrustResultType)trustResultType
{
    ...
    [OTRCertificatePinning addCertificateData:certifcateData withHostName:hostname];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self startConnection];
    });
}
...
-(void)sendReadReceipt:(id<OTRMessageProtocol>)message {
    
    XMPPMessage *receiptMessage = nil;
    if ([message isKindOfClass:[OTRXMPPRoomMessage class]]) {
        OTRXMPPRoomMessage *imessage = [((OTRXMPPRoomMessage *)message) copy];
        NSString * messageID = imessage.xmppId;
        receiptMessage = [XMPPMessage messageWithType:@"groupchat" to:[XMPPJID jidWithString:imessage.roomJID] elementID:messageID];
        if (imessage.senderJID != nil) {
            [receiptMessage addDisplayedChatMarkerWithID:[receiptMessage elementID] sender:imessage.senderJID];
        } else {
            [receiptMessage addDisplayedChatMarkerWithID:[receiptMessage elementID]];
        }
    } else {
        OTRIncomingMessage *imessage = [((OTRIncomingMessage *)message) copy];
        NSString * messageID = imessage.messageId;
        __block OTRXMPPBuddy *buddy = nil;
        [[OTRDatabaseManager sharedInstance].readConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
            buddy = (OTRXMPPBuddy *)[message threadOwnerWithTransaction:transaction];
        }];
        ...
        receiptMessage = [XMPPMessage messageWithType:@"chat" to:buddy.bareJID elementID:messageID];
        [receiptMessage addDisplayedChatMarkerWithID:[receiptMessage elementID]];
    }
    
    XMPPJID *jid = [XMPPJID jidWithString:self.account.username resource:self.account.resource];
    [receiptMessage addAttributeWithName:@"from" stringValue:jid.full];
    [receiptMessage addStorageHint:XMPPMessageStorageStore];
