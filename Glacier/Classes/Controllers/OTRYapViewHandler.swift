...
@objc public protocol OTRYapViewHandlerDelegateProtocol:NSObjectProtocol {
    ...
    @objc optional func mappingsUpdated(_ handler:OTRYapViewHandler)
}
...
public class OTRYapViewHandler: NSObject {
    ...
    var extNotificationToken:NSObjectProtocol? = nil
    ...
    @objc public init(databaseConnection:YapDatabaseConnection, databaseChangeNotificationName:String = DatabaseNotificationName.LongLivedTransactionChanges) {
        ...
        self.extNotificationToken = NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: DatabaseNotificationName.LongLivedExternalTransactionChanges), object: self.databaseConnection, queue: OperationQueue.main) {[weak self] (notification) -> Void in
            self?.updateMappings(notification)
        }
    }
    ...
    deinit {
        ...
        if let exttoken = self.extNotificationToken {
            NotificationCenter.default.removeObserver(exttoken)
        }
    }
    ...
    @objc func updateMappings(_ notification:Notification) {
        self.databaseConnection.read({ (transaction) in
            self.mappings?.update(with: transaction)
        })
    }
