...
public class PushInfo: NSObject {
    ...
    let backgroundFetchPermitted = true
...
open class PushController: NSObject, PushControllerProtocol {
    ...
    @objc open func deactivate(completion: (()->())?, callbackQueue: DispatchQueue?) {
        let hasStorage = (_storage != nil)
    ...
    @objc public func reset(completion: (()->())?, callbackQueue: DispatchQueue?) {
    ...
    @objc public static func setPushPreference(_ preference: PushPreference) {
        var bool = false
    ...
    @objc public static func registerForPushNotifications() {
        if #available(iOS 10.0, *) {
            ...
            Application.shared.registerUserNotificationSettings(notificationSettings)
        }
    }
    
    @objc public static func canReceivePushNotifications() -> Bool {
        var isEnabled = false
        if let settings = Application.shared.currentUserNotificationSettings {
            isEnabled = settings.types != UIUserNotificationType()
        }
        return isEnabled
    }
    
    @objc public func setPushToken(_ token: String!) -> Void {
        pushToken = token
    }
    ...
    @objc public func setVoipToken(_ token: String!) -> Void {
        voipToken = token
    }
