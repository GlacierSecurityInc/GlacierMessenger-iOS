...
@objc open class OTRXMPPRoom: OTRYapDatabaseObject {
    ...
    static var seedNum:Int = -1
    @objc open var avatarSeedNum = -1
    @objc public func nextAvatarNum(with seed: Int) -> Int {
        if (OTRXMPPRoom.seedNum < 0) {
            if (seed >= 0) {
                OTRXMPPRoom.seedNum = seed+1
                return OTRXMPPRoom.seedNum
            }
#if !TARGET_IS_EXTENSION
            OTRXMPPRoom.seedNum = OTRGroupAvatarGenerator.randomSeedNum(withSeed: uniqueId)
            return OTRXMPPRoom.seedNum
#endif
        }
        OTRXMPPRoom.seedNum = OTRXMPPRoom.seedNum+1
        return OTRXMPPRoom.seedNum
    }
...
extension OTRXMPPRoom:OTRThreadOwner {
    ...
    // expiration is only for individual conversations
    ...
    public var threadName: String {
        return self.roomName ?? self.subject ?? self.jid ?? ""
    }
    ...
    public var avatarImage: UIImage {
        ...
        if let image = OTRImages.image(withIdentifier: self.uniqueId) {
            ...
            let seedNum = self.avatarSeedNum
            
            if let image = OTRGroupAvatarGenerator.avatarImage(withSeedNum: seedNum, width: 100, height: 100) {
                OTRImages.setImage(image, forIdentifier: self.uniqueId)
                return image
            } else {
...
#else
        return UIImage(named: "glacier", in: GlacierInfo.resourcesBundle, compatibleWith: nil)!
#endif
    }
...
public extension OTRXMPPRoom {
    ...
    @objc var bookmark: XMPPConferenceBookmark? {
        guard let jid = roomJID else { return nil }
        var bookName = self.subject
        if let name = self.roomName {
            bookName = name
        }
        let bookmark = XMPPConferenceBookmark(jid: jid, bookmarkName: bookName, nick: nil, autoJoin: true)
        return bookmark
    }
...
extension OTRXMPPRoom: YapDatabaseRelationshipNode {
    ...
    /// return the OTRXMPPRoomOccupant.uniqueId for all room occupants
    public static func allOccupantKeys(roomUniqueId: String, collectionName: String, transaction: YapDatabaseReadTransaction) -> [String] {
        var occupants: [String] = []
        guard let relationshipTransaction = transaction.ext(DatabaseExtensionName.relationshipExtensionName.name()) as? YapDatabaseRelationshipTransaction else {
            return []
        }
        relationshipTransaction.enumerateEdges(withName: OTRXMPPRoomOccupant.roomEdgeName, destinationKey: roomUniqueId, collection: collectionName) { (edge, stop) in
            let sourceKey = edge.sourceKey
            //assert(edge.sourceCollection == OTRXMPPRoomOccupant.collection, "Wrong collection!")
            occupants.append(sourceKey)
        }
    ...
    @objc public func allOccupants(_ transaction: YapDatabaseReadTransaction) -> [OTRXMPPRoomOccupant] {
