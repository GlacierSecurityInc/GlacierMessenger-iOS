...
#if !TARGET_IS_NOTIFICATION
    if(audioRange.location == 0 || [filename.pathExtension isEqualToString:@"m4a"]) {
        mediaClass = [OTRAudioItem class];
    } else if (imageRange.location == 0) {
        mediaClass = [OTRImageItem class];
    } else if (videoRange.location == 0 || [filename.pathExtension isEqualToString:@"MOV"] || [filename.pathExtension isEqualToString:@"mov"]) {
        mediaClass = [OTRVideoItem class];
    } else if (htmlRange.location == 0) {
        mediaClass = [OTRHTMLItem class];
    } else {
        mediaClass = [OTRFileItem class]; //mime type likely starts with application for PDF, doc, or text for plain text
    }
#endif
...
- (UIView*) errorView {
    ...
    if (message.messageError) {
        MediaDownloadView *downloadView = [MediaDownloadView glacierViewFromNib];
        if (!downloadView) {
            return nil;
        }
...
- (NSString*) displayText {
    ...
    if ([self isKindOfClass:[OTRImageItem class]]) {
        item = [self getImageItemDisplayText];
    ...
    } else if ([self isKindOfClass:[OTRFileItem class]]) {
        item = [NSString stringWithFormat:@"📁 %@", FILE_MESSAGE_STRING()];
    } else {
#endif
...
#if !TARGET_IS_EXTENSION
- (NSString *) getImageItemDisplayText {
    if ([self isKindOfClass:[OTRImageItem class]]) {
        id<OTRDownloadMessage> otrd = [self downloadMessage];
        NSString *origtext = [otrd messageOriginalText];
        if (origtext && [origtext hasPrefix:@"geo"]) {
            return [NSString stringWithFormat:@"%@ Location received", [NSString fa_stringForFontAwesomeIcon:FAIconMapMarker]];
        }
    }
    return [NSString stringWithFormat:@"📷 %@", PICTURE_MESSAGE_STRING()];
}
#endif
