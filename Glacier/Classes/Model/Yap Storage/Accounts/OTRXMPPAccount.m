...
static NSUInteger const OTRDefaultPortNumber = 80; //5222;
...
- (instancetype) initWithUsername:(NSString *)username accountType:(OTRAccountType)accountType {
    if (self = [super initWithUsername:username accountType:accountType]) {
        ...
        _domain = [[self class] defaultHost];
        self.autologin = YES;
        self.rememberPassword = YES;
        self.bypassNetworkCheck = YES;
    }
    return self;
}
...
{
    return [UIImage imageNamed:OTRXMPPImageName inBundle:[GlacierInfo resourcesBundle] compatibleWithTraitCollection:nil];
}
...
{
    return [GlacierInfo defaultHost];
}
...
+ (NSString * )newResource
{
    ...
    return [NSString stringWithFormat:@"%@%d",[GlacierInfo xmppResource],r];
}
