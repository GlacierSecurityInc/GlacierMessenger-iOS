...
@objc open class OTRXMPPRoomMessage: OTRYapDatabaseObject {
    ...
    @objc open var receiverJID:String? //full jid for MAM comparison
    ...
    @objc open var originalText:String?
    @objc open var expires:String?
    @objc open var memberUpdate = false
    @objc open var systemUpdate = false
    @objc open var markable = false
    @objc open var messageDate = Date.distantPast
    @objc open var messageReadDate = Date.distantPast
...
public extension OTRXMPPRoomMessage {
    convenience init(message: XMPPMessage, delayed: Date?, room: OTRXMPPRoom, transaction: YapDatabaseReadTransaction) {
        self.init()
        ...
        if let x = message.element(forName: "x", xmlns: XMPPMUCUserNamespace),
            ...
            realJID = jid
        } else if let addresses = message.element(forName: "addresses", xmlns: "http://jabber.org/protocol/address"),
            let address = addresses.element(forName: "address"),
            let jidString = address.attribute(forName: "jid")?.stringValue,
            let type = address.attribute(forName: "type")?.stringValue,
            let jid = XMPPJID(string: jidString) {
            if (type == "ofrom") {
                realJID = jid
            }
...
extension OTRXMPPRoomMessage:OTRMessageProtocol {
    ...
    public func duplicateMessage() -> OTRMessageProtocol {
        ...
        newMessage.originalText = self.originalText
        newMessage.expires = self.expires
    ...
    public var isMessageDisplayed: Bool {
        return false //change this when we figure out how
    }
    ...
    public var messageOriginalText: String? {
        return self.originalText
    }
    
    public var messageExpires: String? {
        return self.expires
    }
    ...
    public var isMessageIncomingOrDifferentDevice: Bool {
        if (self.isMessageIncoming) {
            return true
        } else if (self.receiverJID != nil && self.realJID?.full != nil && self.receiverJID != self.realJID?.full) {
            return true
        }
...
public class OTRGroupDownloadMessage: OTRXMPPRoomMessage, OTRDownloadMessage {
    ...
    public static func download(withParentMessage parentMessage: OTRMessageProtocol, url: URL) -> OTRDownloadMessage {
        ...
        download.originalText = parentMessage.messageOriginalText
        download.expires = parentMessage.messageExpires
...
extension OTRXMPPRoomMessage: OTRDownloadMessageProtocol {
    ...
    public func existingDownloads(with transaction: YapDatabaseReadTransaction) -> [OTRDownloadMessage] {
        guard self.isMessageIncomingOrDifferentDevice else {
            return []
        }
        var downloads: [OTRDownloadMessage] = []
...
public extension OTRXMPPRoomMessage {
    ...
    func room(_ transaction: YapDatabaseReadTransaction) -> OTRXMPPRoom? {
    ...
    func allBuddyKeysForOutgoingMessage(_ transaction: YapDatabaseReadTransaction) -> [String] {
    ...
    func roomFromShare(_ transaction: YapDatabaseReadTransaction) -> OTRXMPPRoom? {
        return transaction.object(forKey: self.threadId, inCollection: "Glacier.OTRXMPPRoom") as? OTRXMPPRoom
    }
    
    /// Getting all buddies for a room message to faciliate OMEMO group encryption
    func allBuddyKeysForOutgoingShareMessage(_ transaction: YapDatabaseReadTransaction) -> [String] {
        guard let roomId = roomUniqueId else {
            return []
        }
        
        let buddyKeys = OTRXMPPRoom.allOccupantKeys(roomUniqueId: roomId, collectionName:"Glacier.OTRXMPPRoom", transaction: transaction).compactMap
        {
            transaction.object(forKey: $0, inCollection: "Glacier.OTRXMPPRoomOccupant") as? OTRXMPPRoomOccupant
        }.compactMap {
            $0.buddyUniqueId
        }
...
public extension OTRXMPPRoomMessage {
    ...
    @objc static func handleDeliveryReceiptResponse(message: XMPPMessage, writeConnection: YapDatabaseConnection) {
    ...
    @objc static func handleDeliveryReceiptRequest(message: XMPPMessage, xmppStream:XMPPStream) {
...
public extension XMPPRoom {
    @objc func sendRoomMessage(_ message: OTRXMPPRoomMessage) {
