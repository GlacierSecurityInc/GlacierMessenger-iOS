...
extension NotificationType: RawRepresentable {
    public init?(rawValue: String) {
        ...
        } else if rawValue == kNotificationTypeCallAccept {
            self = .callAccept
        } else if rawValue == kNotificationTypeCallReject {
            self = .callReject
        } else if rawValue == kNotificationTypeCallBusy {
            self = .callBusy
        } else if rawValue == kNotificationTypeCallCancel {
            self = .callCancel
        } else {
            return nil
        }
    ...
    public var rawValue: String {
        ...
        case .callAccept:
            return kNotificationTypeCallAccept
        case .callReject:
            return kNotificationTypeCallReject
        case .callBusy:
            return kNotificationTypeCallBusy
        case .callCancel:
            return kNotificationTypeCallCancel
        }
    }
        ...
        if error.domain == GCDAsyncSocketErrorDomain,
            ...
            case .otherError:
                ...
                if let certData = error.userInfo[OTRXMPPSSLCertificateDataKey] as? Data,
                    ...
                    if hostname == OTRSecrets.defaultPublicHost() || hostname == OTRSecrets.defaultHost() || hostname.hasSuffix(OTRSecrets.defaultPublicHost()) || hostname.hasSuffix(OTRSecrets.altPublicHost()){
