...
@property (assign, nonatomic) BOOL doubleDone; // timer flag to handle double notifications
//@property (nonatomic, strong) NSTimer *backgroundTimer;
//@property (nonatomic) UIBackgroundTaskIdentifier backgroundTask;

// test to see if we can get to server (due to VPN) before attempting login
...
//    self.window.rootViewController = nav;
    ...
    self.doubleDone = NO; // to handle double notifications on login
...
- (void)applicationWillResignActive:(UIApplication *)application
{
    ...
    self.splitViewCoordinator.splitViewController.preferredDisplayMode = UISplitViewControllerDisplayModePrimaryHidden;
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    // stop testing network but get ready for when we come back up
    [self.networkTester reset];
    self.enteringForeground = NO;
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:OTRXMPPLoginStatusNotificationName object:self userInfo:nil];
    });
    ...
    [[OTRProtocolManager sharedInstance] disconnectAllAccounts]; // logout when backgrounded
}
...
// open the OpenVPN app if possible
- (void) gotoOpenVPN {
...
- (void) networkConnectionStatusChange:(NSNotification*)notification {
    ...
    if (_networkStatus == OTRNetworkConnectionStatusConnecting || _networkStatus == OTRNetworkConnectionStatusUnknown) {
        return;
    }
    
    // added enteringForeground (sometimes this occurs before Active)
...
{
    //self.splitViewCoordinator.splitViewController.preferredDisplayMode = UISplitViewControllerDisplayModePrimaryOverlay;
    self.splitViewCoordinator.splitViewController.preferredDisplayMode = UISplitViewControllerDisplayModeAllVisible;
...
- (void)applicationWillTerminate:(UIApplication *)application
{
    ...
    [self.conversationViewController teardownCognito];
...
-(void)application:(UIApplication *)application performFetchWithCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler {
    //[self autoLoginFromBackground:YES]; // to avoid logging in from background fetch
