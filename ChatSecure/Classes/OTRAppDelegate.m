...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // removed because we set this app wide
    //[OTREncryptionManager setFileProtection:NSFileProtectionCompleteUntilFirstUserAuthentication path:NSTemporaryDirectory()];
    ...
    [self performSelector:@selector(migrateAccountPassword:) withObject:nil afterDelay:8.0];
    
    return YES;
}

// previously account passwords were stored via SAMKeychain with kSecAttrAccessibleWhenUnlockedThisDeviceOnly, need to be updated for calls to work when locked
- (void) migrateAccountPassword:(id)sender {
    ...
    [accounts enumerateObjectsUsingBlock:^(OTRAccount * account, NSUInteger idx, BOOL *stop) {
        NSString *passvalue = account.password;
        BOOL result = [account removeKeychainPassword:nil];
        if (result) {
