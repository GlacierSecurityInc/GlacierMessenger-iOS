...
public extension YapDatabaseReadTransaction {
    ...
    @objc func enumerateMessages(elementId:String?, originId: String?, stanzaId:String?, block:@escaping (_ message:OTRMessageProtocol,_ stop:UnsafeMutablePointer<ObjCBool>) -> Void) {
    ...
    @objc func enumerateSessions(accountKey:String, signalAddressName:String, block:@escaping (_ session:OTRSignalSession,_ stop:UnsafeMutablePointer<ObjCBool>) -> Void) {
...
//    }
    ...
    @objc func numberOfUnreadMessages() -> UInt {
        guard let secondaryIndexTransaction = self.ext(SecondaryIndexName.messages) as? YapDatabaseSecondaryIndexTransaction else {
            return 0
        }
    ...
    @objc func allUnreadMessagesForThread(_ thread:OTRThreadOwner) -> [OTRMessageProtocol] {
...
public extension YapDatabaseReadTransaction {
    ...
    @objc func enumerateUnreadMessages(_ block:@escaping (_ message:OTRMessageProtocol,_ stop:UnsafeMutablePointer<ObjCBool>) -> Void) {
        guard let secondaryIndexTransaction = self.ext(SecondaryIndexName.messages) as? YapDatabaseSecondaryIndexTransaction else {
            return
        }
        let queryString = "Where \(MessageIndexColumnName.isMessageRead) == 0"
...
public extension YapDatabaseReadTransaction {
    ...
    @objc func enumerateUnfinishedDownloads(_ block:@escaping (_ mediaItem:OTRMediaItem,_ stop:UnsafeMutablePointer<ObjCBool>) -> Void) {
