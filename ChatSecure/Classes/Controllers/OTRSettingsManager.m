...
@interface OTRSettingsManager () <TimerPickerDelegate> // for global expiration timer
...
- (void) populateSettings
{
    ...
    NSArray *chatSettings = @[deletedDisconnectedConversations,self.globalMessageTimerSetting,clearHistorySetting];
    ...
    OTRViewSetting *displayNameSetting = [[OTRViewSetting alloc] initWithTitle:@"Change Display Name" description:nil viewControllerClass:nil];
    displayNameSetting.actionBlock = ^void(id sender) {
        if ([OTRAccountsManager allAccounts].count != 0) {
            [self handleChangeDisplayName:sender];
        }
    };
    [newSettingsDictionary setObject:displayNameSetting forKey:@"kOTRSettingKeyDisplayName"];
    ...
    [newSettingsDictionary setObject:supportSetting forKey:@"kOTRSettingKeySupport"];
    
    OTRViewSetting *aboutSetting = [[OTRViewSetting alloc] initWithTitle:@"About" description:nil viewControllerClass:nil];
    aboutSetting.actionBlock = ^void(id sender) {
        if (![sender isKindOfClass:[OTRSettingsViewController class]]) {
            return;
        }
        
        OTRSettingsViewController *sourceVC = sender;
        if ([sourceVC respondsToSelector:@selector(showAboutScreen:)]) {
            [sourceVC showAboutScreen:sender];
        }
    };
    [newSettingsDictionary setObject:aboutSetting forKey:@"kOTRSettingKeyAbout"];
    
    NSArray * otherSettings = @[displayNameSetting];
...
- (void) handleClearMessages:(id)sender {
    ...
    UIAlertAction *clear = [UIAlertAction actionWithTitle:@"Clear Now" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [accounts enumerateObjectsUsingBlock:^(OTRAccount * account, NSUInteger idx, BOOL *stop) {
            YapDatabaseConnection *rwDatabaseConnection = [OTRDatabaseManager sharedInstance].readWriteDatabaseConnection;
