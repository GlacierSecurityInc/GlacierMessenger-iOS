...
public class FileTransferManager: NSObject, OTRServerCapabilitiesDelegate {
    ...
    @objc public init(connection: YapDatabaseConnection,
        ...
        self.uploadDomain = OTRSecrets.defaultHost()
        super.init()
    ...
    @objc public init(connection: YapDatabaseConnection,
        ...
        super.init()
        if allowSelfSigned {
            self.acceptSelfSignedSSLCerts() // enable self-signed certs only if we are in closed network
        }
    ...
    @objc public func downloadAttachment(url: URL, key: String, iv: String, tag: String,
        ...
        internalQueue.async {
            ...
            var tempurl = url
            if let temphost = url.host {
                if temphost != self.uploadDomain, temphost.contains(OTRSecrets.defaultHost()) {
                    if var components = URLComponents(url: tempurl, resolvingAgainstBaseURL: true) {
                        components.host = self.uploadDomain
                        tempurl = components.url!
                    }
                }
            }
    ...
    @objc public func send(image: UIImage, thread: OTRThreadOwner) {
        internalQueue.async {
            ...
            // changed to low  to compress more
    ...
    private func newOutgoingMessage(to thread: OTRThreadOwner, mediaItem: OTRMediaItem) -> OTRMessageProtocol? {
        if let buddy = thread as? OTRBuddy {
            ...
            // check global expiration timer if time not set on message
...
extension FileTransferManager {
    ...
    private func downloadMedia(_ downloadMessage: OTRDownloadMessage) {
        guard var tempurl = downloadMessage.downloadableURL else {
            DDLogWarn("Attempted to download message but couldn't parse a URL \(downloadMessage)")
            return
        }
        
        if let temphost = tempurl.host {
            if temphost != self.uploadDomain, temphost.contains(OTRSecrets.defaultHost()) {
                if var components = URLComponents(url: tempurl, resolvingAgainstBaseURL: true) {
                    components.host = self.uploadDomain
                    tempurl = components.url!
...
public extension FileTransferManager {
    ...
    @objc static func shouldDisplayMessage(_ message: OTRMessageProtocol, transaction: YapDatabaseReadTransaction) -> Bool {
        ...
        // so media messages don't show as link in group
        if messageText.isSingleURLOnly,
