...
extension UIImage {
    enum DataSize {
     ...
     struct Quality {
        ...
        static let low = Quality(initial: 0.3, decrementFactor: 0.3) // compress more
    ...
    func jpegData(dataSize: DataSize,
                  ...
                  jpeg: Quality = Quality.low, // Quality.medium,
...
public class FileTransferManager: NSObject, OTRServerCapabilitiesDelegate {
    ...
    @objc public init(connection: YapDatabaseConnection,
        ...
        super.init()
        self.acceptInvalidSSLCerts() // enable self-signed certs since we are usually in closed network
    ...
    // enable self-signed certs since we are in closed network. Not good if used elsewhere!
...
public extension FileTransferManager {
    ...
    @objc public static func shouldDisplayMessage(_ message: OTRMessageProtocol, transaction: YapDatabaseReadTransaction) -> Bool {
        ...
        // so media messages don't show as a link in group messaging, maybe remove after OMEMO
        if messageText.isSingleURLOnly,
