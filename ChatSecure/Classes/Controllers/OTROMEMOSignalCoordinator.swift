...
@objc open class OTROMEMOSignalCoordinator: NSObject {
 ...
 `  */
    ...
    */
    ...
    open func processKeyData(_ keyData: [OMEMOKeyData], iv: Data, senderDeviceId: UInt32, forJID: XMPPJID, payload: Data?, delayed: Date?, forwarded: Bool, isIncoming: Bool, message: XMPPMessage, originalMessage: XMPPMessage) {
        ...
        for key in keyData where key.deviceId == rid {
            ...
            do {
                ...
                if (letEmKnow && isIncoming && !message.isGroupChatMessage) {
                    ...
                    self.databaseConnection.read { (transaction) in
                        if let buddy = OTRXMPPBuddy.fetchBuddy(jid: addressJID, accountUniqueId: self.accountYapKey, transaction: transaction) {
                            ...
                            if let account = OTRXMPPAccount.fetchObject(withUniqueID: self.accountYapKey, transaction: transaction) {
                                let incoming = OTRIncomingMessage(xmppMessage: message, body: "Message(s) not encrypted for this device and cannot be displayed", account: account, buddy: buddy, capabilities: self.messageStorage.capabilities)
                                if let xmpp = OTRProtocolManager.shared.protocol(for: account) as? XMPPManager {
                                    xmpp.sendDeliveryReceipt(for: incoming)
                                }
                            }
                        } 
                    }
                    ...
                    if (foundbud) {// && !lekList.contains(senderDeviceId)) {
                        //lekList.append(senderDeviceId)
        ...
        guard var aesKey = unencryptedKeyData else {
            handleMissingKeys(message: message, forwarded: forwarded)
            return
        }
        var authTag: Data?
    ...
    func handleMissingKeys(message: XMPPMessage, forwarded: Bool) {
            ...
            if message.isGroupChatMessage {
                if let roomJID = message.from?.bareJID,
                    ...
                    if (!missingKeysSet.contains(missingFrom)) {
                        missingKeysSet.insert(missingFrom)
                        ...
                        if let myid = self.myJID, !publishingMyDevice {
                            publishingMyDevice = true
                ...
                if let missingFrom = message.fromStr,
                    ...
                    self.messageStorage.handleDirectMessage(message, body: "Message(s) not encrypted for this device and cannot be displayed. For more information visit https://glaciersecurity.zendesk.com/hc/en-us/articles/360027880632", original: nil, accountId: self.accountYapKey)
