...
- (void)insertIncomingMessage:(XMPPMessage *)message intoRoom:(XMPPRoom *)room
{
    ...
    [self.databaseConnection asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
        ...
        // share location
        NSString *newMsg = [databaseMessage.messageText copy];
        if ([newMsg hasPrefix:@"geo:"]) {
            NSString* geoless = [newMsg stringByReplacingOccurrencesOfString:@"geo:" withString:@""];
            NSArray *loccomponents = [geoless componentsSeparatedByString:@","];
            if (loccomponents.count == 2) {
                NSString *defaultAccessToken = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"MGLMapboxAccessToken"];
                
                CLLocationCoordinate2D loc2d = CLLocationCoordinate2DMake([loccomponents.firstObject doubleValue], [loccomponents.lastObject doubleValue]);
                
                NSURL *styleURL = [NSURL URLWithString:@"mapbox://styles/mapbox/dark-v9"];
                MBSnapshotCamera *camera = [MBSnapshotCamera cameraLookingAtCenterCoordinate:loc2d
                                            zoomLevel:12];
                
                MBSnapshotOptions *options = [[MBSnapshotOptions alloc] initWithStyleURL:styleURL
                                            camera:camera size:CGSizeMake(375, 667)];
                MBSnapshot *snapshot = [[MBSnapshot alloc] initWithOptions:options
                                            accessToken:defaultAccessToken];
                
                databaseMessage.messageText = [snapshot.url absoluteString];
                //if imageURL.range(of:"https") != nil {
                //    messageString = imageURL.replacingOccurrences(of: "https", with: "aesgcm")
                //}
                
                databaseMessage.originalText = newMsg; 
            }
        }
...
- (void)handleSubscription:(XMPPRoom *)room toJid:(XMPPJID *)tojid withNickname:(NSString *)nickname {
    NSString *accountId = room.xmppStream.tag;
    NSString *rjid = room.roomJID.bare;
    
    if (accountId != nil && rjid != nil) {
        [self.databaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
            OTRXMPPRoom *databaseRoom = [self fetchRoomWithXMPPRoomJID:rjid accountId:accountId inTransaction:transaction];
            if (databaseRoom != nil && ![databaseRoom isMuted]) {
                [room subscribeToRoom:tojid withNick:nickname];
            }
        }];
    }
}
