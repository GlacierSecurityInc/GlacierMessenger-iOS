...
- (void)setupStream
{
    ...
    if (_account != nil && [_account.domain isEqualToString:[OTRSecrets defaultHost]]) {
        allowselfsigned = YES;
    } else if (_account != nil) {
...
- (void)pushModule:(XMPPPushModule*)module readyWithCapabilities:(NSXMLElement *)caps jid:(XMPPJID *)jid {
    ...
    if (!hasPushAccount) {
        // assume that account will be created in a few seconds
        dispatch_async(dispatch_get_main_queue(), ^{
            [self performSelector:@selector(prepareToEnablePush) withObject:nil afterDelay:10.0];
        });
    } else {
        [self prepareToEnablePush];
    }
}
