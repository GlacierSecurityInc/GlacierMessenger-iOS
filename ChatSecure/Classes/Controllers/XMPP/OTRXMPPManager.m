...
- (BOOL)startConnection
{
    ...
    if (![self.xmppStream connectWithTimeout:(NSTimeInterval)5 error:&error])
	{
		[self failedToConnect:error];
...
- (void)xmppRoster:(XMPPRoster *)sender didReceiveRosterItem:(NSXMLElement *)item {
    ...
    [[OTRDatabaseManager sharedInstance].readOnlyDatabaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        ...
        [self.databaseConnection asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
            ...
            NSString *bname = [item attributeStringValueForName:@"name"];
            if (bname) {
                if (![bname isEqualToString:buddy.displayName]) {
                    buddy.displayName = bname;
                }
                if (![bname isEqualToString:buddy.vCardTemp.nickname]) {
                    buddy.vCardTemp.nickname = bname;
                }
            }
