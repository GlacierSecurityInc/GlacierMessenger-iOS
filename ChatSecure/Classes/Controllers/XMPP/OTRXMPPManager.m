...
- (void)setupStream
{
    ...
    NSString *uploaddomain = [OTRSecrets defaultHost];
    if (_account != nil) {
...
- (void)pushModule:(XMPPPushModule*)module readyWithCapabilities:(NSXMLElement *)caps jid:(XMPPJID *)jid {
    ...
    if (token != nil) {
        ...
        NSString *pushjid = [@"p2." stringByAppendingString:[OTRSecrets defaultPublicHost]];
...
- (void)newTrust:(SecTrustRef)trust withHostName:(NSString *)hostname systemTrustResult:(SecTrustResultType)trustResultType
{
    ...
    BOOL glaciercert = [hostname isEqualToString:[OTRSecrets defaultHost]] || [hostname isEqualToString:[OTRSecrets defaultPublicHost]] || [hostname hasSuffix:[OTRSecrets defaultPublicHost]] || [hostname hasSuffix:[OTRSecrets altPublicHost]];
    if (glaciercert) {
        ...
        if (certAlert) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{ 
                [OTRAppDelegate.appDelegate.window.rootViewController presentViewController:certAlert animated:YES completion:nil];
            });
        }
    }
