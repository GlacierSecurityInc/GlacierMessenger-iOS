...
- (void)xmppStream:(XMPPStream *)stream didReceiveMessage:(XMPPMessage *)xmppMessage
{
    ...
    [self.databaseConnection asyncReadWriteWithBlock:^(YapDatabaseReadWriteTransaction *transaction) {
        ...
        if (message.text) {
            ...
            if ([xmppMessage elementForName:@"x" xmlns:@"jabber:x:msgexpire"]) {
                DDXMLElement *expiretime = [xmppMessage elementForName:@"x" xmlns:@"jabber:x:msgexpire"];
                message.expires = [expiretime attributeStringValueForName:@"seconds"];
            }
            ...
            // new messages in archived conversation go to Inbox
            ...
            if (thread && thread.isArchived) {
                ...
                if ([thread isKindOfClass:[OTRBuddy class]]) {
                    OTRBuddy *threadOwner = (OTRBuddy*)thread;
                    [threadOwner saveWithTransaction:transaction];
                }
            }
        }
    }];
