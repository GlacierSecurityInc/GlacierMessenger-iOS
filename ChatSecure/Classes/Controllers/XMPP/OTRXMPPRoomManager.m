...
// placeholder for better tracking room member presence? (next one also)
...
+ (NSXMLElement *)defaultRoomConfiguration:(BOOL)ispublicroom
{
    ...
    if (!ispublicroom) {
        NSXMLElement *maxusersField = [[NSXMLElement alloc] initWithName:@"field"];
        [maxusersField addAttributeWithName:@"var" stringValue:@"muc#roomconfig_maxusers"];
        [maxusersField addChild:[[NSXMLElement alloc] initWithName:@"value" numberValue:@(10)]];
        [form addChild:maxusersField];
    }
