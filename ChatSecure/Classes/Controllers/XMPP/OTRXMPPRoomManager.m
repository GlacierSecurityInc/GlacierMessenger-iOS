...
- (void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence
{
    ...
    if (presenceType != nil && [presenceType isEqualToString:@"error"]) {
        XMPPJID *from = [presence from];
        NSString *fromDomain = from.domain;
        if (fromDomain != nil && [fromDomain hasPrefix:@"conference"]) {
            dispatch_async(dispatch_get_main_queue(), ^(void){
                [self performSelector:@selector(removeRoomOnError:) withObject:from afterDelay:1.0];
            });
        }
    }
}
...
- (void) removeRoomOnError:(XMPPJID*)roomjid  {
    ...
    [self.databaseConnection readWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
        [transaction enumerateKeysAndObjectsInCollection:[OTRXMPPRoom collection] usingBlock:^(NSString * _Nonnull key, id  _Nonnull object, BOOL * _Nonnull stop) {
            if ([object isKindOfClass:[OTRXMPPRoom class]]) {
                OTRXMPPRoom *room = (OTRXMPPRoom *)object;
                if ([room.jid length] && [roomjid.bare isEqualToString:room.jid]) {
        ...
        if ([curRoomArray count]) {
            [[curRoomArray firstObject] removeWithTransaction:transaction];
        }
    }];
    
    NSString *amsg = [NSString stringWithFormat:@"There was a problem joining the %@ room, so it will not be in your conversations list.", roomjid.user];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Problem joining room" message:amsg delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
    [alert show];
}
    ...
    [self.databaseConnection readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        [transaction enumerateKeysAndObjectsInCollection:[OTRXMPPRoom collection] usingBlock:^(NSString * _Nonnull key, id  _Nonnull object, BOOL * _Nonnull stop) {
            ...
            if ([object isKindOfClass:[OTRXMPPRoom class]]) {
                OTRXMPPRoom *room = (OTRXMPPRoom *)object;
                if ([room.jid length]) {
                    [curRoomArray addObject:room.jid];
                }
            }
        }];
    }];
...
- (void) xmppRoom:(XMPPRoom *)room didFetchMembersList:(NSArray *)items {
    ...
    /*NSString *databaseRoomKey = [OTRXMPPRoom createUniqueId:self.xmppStream.tag jid:[room.roomJID bare]];
    [self.databaseConnection readWriteWithBlock:^(YapDatabaseReadWriteTransaction * _Nonnull transaction) {
        OTRXMPPRoom *xroom = [OTRXMPPRoom fetchObjectWithUniqueID:databaseRoomKey transaction:transaction];
        if (xroom) {
            [self clearOccupantRolesInRoom:xroom withTransaction:transaction];
        }
    }];*/
