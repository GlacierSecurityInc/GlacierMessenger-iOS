...
NSString *const OTRXMPPImageName              = @"glacier.png";
...
- (BOOL) removeKeychainPassword:(NSError**)error {
    ...
    if (!result) {
        ...
        //[GlacierLog glogWithLogMsg:@"Error deleting account password from keychain" detail:self.uniqueId];
    } else {
        DDLogInfo(@"Password for %@ deleted from keychain.", self.username);
        //[GlacierLog glogWithLogMsg:@"Account Password deleted from keychain" detail:self.username];
    }
    if (error) {
        *error = internalError;
    }
...
- (void)setPassword:(NSString *) password {
    ...
    if (!result) {
        ...
        //[GlacierLog glogWithLogMsg:@"Error saving account password to keychain" detail:self.uniqueId];
    } else {
        //[GlacierLog glogWithLogMsg:@"Account Password set" detail:self.uniqueId];
    }
}
...
- (NSString *)password {
    ...
    if (error) {
        ...
        //[GlacierLog glogWithLogMsg:@"Error retrieving account password from keychain" detail:self.uniqueId]; 
    }
    return password;
}
