...
- (void) createFooterStack {
    ...
    UIImage *logoutImage = [UIImage imageNamed:@"glacier_red_logout" inBundle:[OTRAssets resourcesBundle] compatibleWithTraitCollection:nil];
    ...
    [logoutButton.heightAnchor constraintEqualToConstant:70].active = true;
...
-(void) doLogout:(id)sender {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (account) {
            ...
            // might not need the above if we wipe the database completely
            // call clearDirectory? maybe a notification
            //[NSFileManager.defaultManager clearDirectory];
        }
        [[[OTRAppDelegate appDelegate] conversationViewController] resetAfterLogout:welcomeNavController];
