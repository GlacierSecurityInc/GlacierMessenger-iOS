...
- (void) groupButtonPressed:(id)sender {
    ...
    NSMutableAttributedString *atrStr = [[NSMutableAttributedString alloc] initWithString:@"\nOpen groups have no user limit and store group chat history\n\nInvite only groups have a 10 user recommended limit and are managed by the owner" attributes:@{NSParagraphStyleAttributeName:paraStyle,NSFontAttributeName:[UIFont systemFontOfSize:13.0]}];
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"New Group" message:@"Open groups have no user limit and store group chat history\nInvite only groups have a 10 user recommended limit and are managed by the owner" preferredStyle:UIAlertControllerStyleAlert];
    [alert setValue:atrStr forKey:@"attributedMessage"];
    UIAlertAction *privateAction = [UIAlertAction actionWithTitle:@"Invite Only" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void) joinGroup:(id)sender {
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"OTRJoinGroup" bundle:[OTRAssets resourcesBundle]];
