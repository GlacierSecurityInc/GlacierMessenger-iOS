...
- (void)viewDidLoad
{
    ...
    [cancelBarButtonItem setTintColor:[UIColor blackColor]];
    [self.groupBarButtonItem setTintColor:[UIColor blackColor]];
...
- (void) groupButtonPressed:(id)sender {
    ...
    NSMutableAttributedString *atrStr = [[NSMutableAttributedString alloc] initWithString:@"\nOpen groups have no user limit and store group chat history\n\nInvite only groups have a 20 user recommended limit and are managed by the owner" attributes:@{NSParagraphStyleAttributeName:paraStyle,NSFontAttributeName:[UIFont systemFontOfSize:13.0]}];
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"New Group" message:@"Open groups have no user limit and store group chat history\nInvite only groups have a 20 user recommended limit and are managed by the owner" preferredStyle:UIAlertControllerStyleAlert];
