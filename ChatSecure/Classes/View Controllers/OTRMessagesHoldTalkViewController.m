...
- (void)attachmentPicker:(OTRAttachmentPicker *)attachmentPicker shareLocationButtonPressed:(NSURL *)locationURL
{
    ...
    // text of button "locating" vs "share" implement listener
}
...
- (void)addLocationViewBtns:(BOOL)shareable
{
    ...
    //UIBarButtonItem *btnShare = [[UIBarButtonItem alloc] initWithTitle:@"Share Location" style:UIBarButtonItemStylePlain target:self action:@selector(shareLocation)];
    if (shareable) {
        [barItems addObject:self.btnMapShare];
    }
    
    //if (self.locReady) {
    //    btnShare.title = @"Share Location";
    //}
