...
- (void)viewDidLoad
{
    ...
    self.titleImageView.image = [UIImage imageNamed:@"glacier" inBundle:[OTRAssets resourcesBundle] compatibleWithTraitCollection:nil];
    ...
    [shareButtons addObject:[self buttonWithIcon:FAIconEnvelope title:INVITE_LINK_STRING() type:BButtonTypeDefault action:@selector(linkShareButtonPressed:)]];
    [shareButtons addObject:[self buttonWithIcon:FAIconCamera title:SCAN_QR_STRING() type:BButtonTypeDefault action:@selector(qrButtonPressed:)]];
...
- (void) setupServerInfoButton {
    _serverInfoButton = [self buttonWithIcon:FAIconInfoCircle title:    SERVER_INFORMATION_STRING() type:BButtonTypeDefault action:@selector(warningButtonPressed:)];
...
- (void) refreshServerInfoButton {
    ...
    if (self.serverCheck.getCombinedPushStatus == ServerCheckPushStatusBroken) {
        ...
        [self.serverInfoButton addAwesomeIcon:FAIconWarning beforeTitle:YES];
    } else {
        [self.serverInfoButton setTitle:SERVER_INFORMATION_STRING() forState:UIControlStateNormal];
        [self.serverInfoButton addAwesomeIcon:FAIconInfoCircle beforeTitle:YES];
    }
}
