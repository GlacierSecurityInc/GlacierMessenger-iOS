...
- (OTRXMPPAccount *) getFirstAccount {
    __block OTRXMPPAccount *firstAcct;
...
- (void) setSideAccount {
    OTRXMPPAccount *firstAcct = [self getFirstAccount];
    if (firstAcct != nil) {
...
- (void) showOnboardingIfNeeded {
    if (self.hasPresentedOnboarding) { //and not currently in process of waiting for login info
        ...
        // if had time to save but didnt have time to finish creating account...
        // how do we know if this is the case?
        
        // tries to create account first before vpn turned on. After it is turned on, should try again
        // should we ping first? Or create account on successful ping?
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(connectionStateDidChange:) name:OTRXMPPLoginStatusNotificationName object:nil];
...
- (void)settingsButtonPressed:(id)sender
{
    ...
    sideMenuFrame.origin.x = 260-self.sideView.frame.size.width;//-115;
...
- (void)addVPNConnection {
    ...
    if (_accountType != nil && ![_accountType isEqualToString:@"openvpn"]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Account notification" message:@"Your account does not support VPN configurations. Contact your administrator for more information." preferredStyle:UIAlertControllerStyleAlert];
...
- (void)tryLogOut {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Warning" message:@"Are you sure to want to logout?" preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:cancelButtonItem];
    [alert addAction:okButtonItem];
    [self presentViewController:alert animated:YES completion:nil];
}
...
-(void) doLogout {
    ...
    OTRXMPPAccount *account = [self getFirstAccount];
    if (account != nil) {
        [[OTRProtocolManager sharedInstance] removeProtocolForAccount:account];
        
        // trying to alleviate DNZEmptySet crash when logging out. Could be thread timing.
...
- (void) setConnectionStatus {
    ...
    if (firstAcct != nil) {
        ...
        if ([[OTRProtocolManager sharedInstance] existsProtocolForAccount:firstAcct]) {
            id <OTRProtocol> protocol = [[OTRProtocolManager sharedInstance] protocolForAccount:firstAcct];
            OTRProtocolConnectionStatus cstat = [protocol connectionStatus];
            if (cstat == OTRProtocolConnectionStatusConnected) { // || !self.statusUpdatesAllowed) {
