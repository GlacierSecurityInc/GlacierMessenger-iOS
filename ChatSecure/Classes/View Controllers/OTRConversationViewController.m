        ...
        [self.aloginHandler finishConnectingWithAccount:account completion:^(OTRAccount *account, NSError *error) {
            ...
            if (error) {
                ...
                self.retryCtr++;
                    ...
                    NSString *hostname = error.userInfo[OTRXMPPSSLHostnameKey];
                    BOOL glacierpublic = [hostname isEqualToString:[OTRSecrets defaultPublicHost]];
                    
                    if (components.count == 2 && certAlert && !glacierpublic) {
...
- (void) createListObjectsRequest {
    ...
    if (self.bucketOrg) {
        ...
        DDLogWarn(@"Could not createListObjectsRequestWithId due to no Org Id");
    }
}
...
- (void) listObjects:(AWSS3ListObjectsRequest*)listObjectsRequest {
    ...
    [[s3 listObjects:listObjectsRequest] continueWithBlock:^id(AWSTask *task) {
        if (task.error) {
            ...
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"No Account" message:@"We cannot find your account information. Please make sure you entered the correct Org ID. For assistance contact your Glacier account representative." preferredStyle:UIAlertControllerStyleAlert];
            [alert addAction:okButtonItem];
            [self presentViewController:alert animated:YES completion:nil];
        } else {
