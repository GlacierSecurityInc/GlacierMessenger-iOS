...
- (void)connectPhoneCall:(TwilioCall *)call {
    ...
    if (!_currentmgr || !buddy) {
        return;
    }
    
    [self openCallController:buddy.displayName];
    ...
    [callVC doConnectCall:call with:buddy];
}
...
- (void) openCallController:(NSString *)nameTitle {
    ...
    if (appDelegate.callWindow.rootViewController == nil) {
        ...
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"PhoneCall" bundle:[OTRAssets resourcesBundle]];
