    ...
    if ([self validForm]) {
        if (self.existingAccount) {
            ...
            if ([password isEqualToString:self.account.password] && [username isEqualToString:self.account.username]) {
                ...
                // (should only get here through multi taps and a pushView now)
                //[self dismissViewControllerAnimated:YES completion:nil];
...
- (instancetype) initWithAccount:(OTRAccount*)account
{
    ...
    if (self = [super initWithForm:form style:UITableViewStyleGrouped]) {
        ...
        XLFormSectionDescriptor *buttonSection = [XLFormSectionDescriptor formSectionWithTitle:@"Details"];
        XLFormRowDescriptor *manageKeysRow = [XLFormRowDescriptor formRowDescriptorWithTag:kOTRXLFormTestFieldTag rowType:XLFormRowDescriptorTypeButton title:@"Manage My Keys"];
        manageKeysRow.action.formSelector = @selector(manageKeys:);
        [buttonSection addFormRow:manageKeysRow];
        XLFormRowDescriptor *serverInfoRow = [XLFormRowDescriptor formRowDescriptorWithTag:kOTRXLFormTestFieldTag rowType:XLFormRowDescriptorTypeButton title:@"Server Information"];
        serverInfoRow.action.formSelector = @selector(getServerInfo:);
        [buttonSection addFormRow:serverInfoRow];
        [form addFormSection:buttonSection];
    }
    return self;
}

-(void)manageKeys:(XLFormRowDescriptor *)sender
{
    XLFormDescriptor *xlform = [UserProfileViewController profileFormDescriptorForAccount:self.account buddies:@[] connection:OTRDatabaseManager.shared.readWriteDatabaseConnection];
    UserProfileViewController *keyMgmtVC = [[UserProfileViewController alloc] initWithAccountKey:self.account.uniqueId connection:OTRDatabaseManager.shared.readWriteDatabaseConnection form:xlform];
    [self.navigationController pushViewController:keyMgmtVC animated:YES];
}
