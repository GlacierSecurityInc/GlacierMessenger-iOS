...
- (void)setupInfoButton {
    ...
    if ([self isGroupChat]) {
        ...
        [pinButton setTintColor:lblColor];
        [pinButton setTitleColor:lblColor forState:UIControlStateNormal];
...
- (void)didSelectPhoneButton:(id)sender {
    ...
    [xmpp.callManager makeCall:buddy.username];
}
...
- (BOOL)isFailedDecryptMessage:(id <OTRMessageProtocol>)message {
    if ([[message messageText] hasPrefix:@"Couldn't decrypt"] || [[message messageText] hasPrefix:@"Glacier Assistant:"] || [[message messageText] hasPrefix:@"Message(s) not encrypted"]) {
        return YES;
    }
    return NO;
}
