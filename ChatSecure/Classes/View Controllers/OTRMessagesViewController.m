...
@property (nonatomic, weak) id otrXMPPLoginStatusNotificationObject;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.otrXMPPLoginStatusNotificationObject = [[NSNotificationCenter defaultCenter] addObserverForName:OTRXMPPLoginStatusNotificationName object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
        [self setConnectionStatus];
    }];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self.otrXMPPLoginStatusNotificationObject];
...
- (void)didTapAvatar:(id<OTRMessageProtocol>)message sender:(id)sender {
    ...
    if ([actions count] > 1) { // changed from 0 to 1 because should always have Cancel action
...
-(void)updateEncryptionState
{
    if ([self isGroupChat]) {
        ...
        [self.readOnlyDatabaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
            ...
            BOOL canSendMedia = YES; // because we are designed to run on our own server
        ...
        [self.readOnlyDatabaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
            ...
            BOOL canSendMedia = YES; // because we are designed to run on our server
            // Check for XEP-0363 HTTP upload
            // TODO: move this check elsewhere so it isnt dependent on refreshing crypto state
            if (xmpp != nil && xmpp.fileTransferManager.canUploadFiles) {
                canSendMedia = YES;
...
- (void)didPressSendButton:(UIButton *)button withMessageText:(NSString *)text senderId:(NSString *)senderId senderDisplayName:(NSString *)senderDisplayName date:(NSDate *)date
{
    ...
    [self.readOnlyDatabaseConnection readWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        ...
        if ([message isKindOfClass:[OTROutgoingMessage class]]) {  // 4.2.1 and Swift 4
    ...
    if (outgoingMessage) { // don't like this, could cause issue with CS 4.2.1
...
- (NSAttributedString *)collectionView:(JSQMessagesCollectionView *)collectionView attributedTextForCellBottomLabelAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([message isKindOfClass:[OTROutgoingMessage class]]) {
        NSDate *date = [message messageDate];
        if (date != nil && ![message isKindOfClass:[OTRXMPPRoomMessage class]]) {
...
- (NSTimeInterval) getTimeUntilExpires:(nonnull id<OTRMessageProtocol>)message {
    if (message.messageExpires) {
        NSDate *expDate = [[message messageDate] dateByAddingTimeInterval:[message.messageExpires intValue]];
...
- (void)layoutJIDForwardingHeader {
    if (self.jidForwardingHeaderView != nil) {
        ...
        self.additionalContentInset = UIEdgeInsetsMake(height, 0, 0, 0); // with JSQ update
    }
}
...
- (void)setupWithBuddies:(NSArray<NSString *> *)buddies accountId:(NSString *)accountId name:(NSString *)name
{
    ...
    if (service != nil) {
        XMPPJID *roomJID = [XMPPJID jidWithString:[NSString stringWithFormat:@"%@@%@",name,service]];
