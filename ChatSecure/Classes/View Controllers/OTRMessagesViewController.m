...
@interface OTRMessagesViewController () <UITextViewDelegate, OTRAttachmentPickerDelegate, OTRYapViewHandlerDelegateProtocol, OTRMessagesCollectionViewFlowLayoutSizeProtocol, OTRRoomOccupantsViewControllerDelegate, JSQMessagesComposerTextViewPasteDelegate> {
...
- (void)viewDidLoad
{
    ...
    // should already have account to be in Messages view
...
- (BOOL)composerTextView:(JSQMessagesComposerTextView *)textView shouldPasteWithSender:(id)sender
{
    if ([UIPasteboard generalPasteboard].image) {
        ...
        return NO;
    } else { // placeholder for trying to send other file types, work in progress
        ...
        if (moreData) {
            ...
            if (testdata) {
                ...
                DDLogInfo(@"testing string %@ and %@", teststring, testString2);
            ...
            DDLogInfo(@"testing with length %lu", (unsigned long)[moreData length]);
        }
    }
    return YES;
}
...
- (void)didTapAvatar:(id<OTRMessageProtocol>)message sender:(id)sender {
    ...
    if ([actions count] > 1) { // should always have cancel action
...
- (void)sendPDFFile:(NSData *)filedata {
    ...
    [xmpp.fileTransferManager sendWithFileData:filedata thread:thread];
}
