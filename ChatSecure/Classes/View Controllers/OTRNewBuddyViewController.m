...
-(IBAction)doneButtonPressed:(id)sender
{
    if ([self checkFields]) {
        ...
        if (components.count != 2) {
            NSString *acctdom = [NSString stringWithFormat: @"%@@%@", [components firstObject], [OTRXMPPAccount defaultHost]];
            [self.accountNameTextField setText:acctdom];
            newBuddyAccountName = acctdom;
        }
